								...
								if (loginResult) {
									...
									if (!data.result.errors) {
										...
										document.addEventListener("deviceready",
												nowLoginInfoSelect(loginId, loginPass,
														tokenID), false);
									} else {
										busy.hide();
		...
		tx.executeSql("select * from user where userid=?;", [ loginId ],
				function(tx, res) {
					...
					if (userSelectResult == 1) {
						...
						document.addEventListener("deviceready",
								currentUserUpdate(loginId), false);
...
function currentUserUpdate(loginId) {
	...
	db
			.transaction(function(tx) {
				tx
						.executeSql(
								"UPDATE user SET currentuser = 1 WHERE userid = ?",
								[ loginId ],
								function(tx, res) {
									console.log("커런트유저  update 처리시작");
...
function loginFuncionSelect() {
	...
	db
			.transaction(function(tx) {
				// tx.executeSql("select *, SUM(case when read=0 then 1 else 0
				// end) as total from message where type=0 or type=1 or type=2
				// or type=3 group by category ",[],
				// function(tx, res) {
								...
								function(tx, res) {
									...
									if (selectLength == 0) {
										...
										for (var i = 0; i < selectLength; i++) {
											...
											console
													.log(contentResult.notification.contentText);
